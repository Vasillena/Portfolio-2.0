...
type SocialIcon = "Facebook" | "Instagram" | "LinkedIn" | "GitHub" | "Behance";

const socials: { title: string; href: string; icon: SocialIcon }[] = [
  {
    title: "Facebook",
    href: "https://www.facebook.com/vasilena.yoneva",
    icon: "Facebook",
  ...
  { title: "Github", href: "https://github.com/Vasillena", icon: "GitHub" },
  {
    title: "Behance",
    href: "https://www.behance.net/vasilenayoneva",
    icon: "Behance",
  ...
  return (
    <div className="hidden absolute left-1/2 bottom-10 ml-4 xl:flex flex-col gap-4">
      {socials.map(({ title, href, icon }) => (
        <Icon key={title} title={title} href={href} icon={icon} />
      ))}
    </div>
  );
}
